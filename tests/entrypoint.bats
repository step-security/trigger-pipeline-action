...
@test "Prints error and fails if \${{ inputs.build_meta_data }} is not valid JSON" {
  export INPUT_BUILDKITE_API_ACCESS_TOKEN="123"
  export INPUT_PIPELINE="my-org/my-pipeline"
  export INPUT_BUILD_META_DATA="broken"
  export GITHUB_EVENT_NAME="create"
...
@test "Shows StepSecurity banner on every run" {

  run "${PWD}"/entrypoint.sh

  assert_output --partial "StepSecurity Maintained Action"
  assert_output --partial "Secure drop-in replacement for buildkite/trigger-pipeline-action"
  assert_failure
}

@test "Shows free message for public repo" {

  export REPO_PRIVATE=false

  run "${PWD}"/entrypoint.sh

  assert_output --partial "✓ Free for public repositories"
  assert_failure
}

@test "Does not show free message for private repo" {

  export REPO_PRIVATE=true

  stub curl "--max-time 3 -s -w %{http_code} -X POST -H * -d * https://agent.api.stepsecurity.io/v1/github/buildkite/test-repo/actions/maintained-actions-subscription -o /dev/null : echo 200"

  run "${PWD}"/entrypoint.sh

  refute_output --partial "✓ Free for public repositories"
  assert_failure

  unstub curl
}

@test "Exits with error when subscription API returns 403 for private repo" {

  export REPO_PRIVATE=true

  stub curl "--max-time 3 -s -w %{http_code} -X POST -H * -d * https://agent.api.stepsecurity.io/v1/github/buildkite/test-repo/actions/maintained-actions-subscription -o /dev/null : echo 403"

  run "${PWD}"/entrypoint.sh

  assert_output --partial "This action requires a StepSecurity subscription for private repositories"
  assert_failure

  unstub curl
}

@test "Continues when subscription API times out for private repo" {
  ...
  stub curl \
    "--max-time 3 -s -w %{http_code} -X POST -H * -d * https://agent.api.stepsecurity.io/v1/github/buildkite/test-repo/actions/maintained-actions-subscription -o /dev/null : exit 1" \
    "--silent --show-error --write-out %{http_code} --output * -X POST -H \"Authorization: Bearer 123\" -d '$EXPECTED_JSON' https://api.buildkite.com/v2/organizations/my-org/pipelines/my-pipeline/builds : output_file=\"\"; for i in \"\$@\"; do if [ \"\$output_file\" = \"next\" ]; then echo '$RESPONSE_JSON' > \"\$i\"; break; fi; [ \"\$i\" = \"--output\" ] && output_file=\"next\"; done; echo 200"

  run "${PWD}"/entrypoint.sh

  assert_output --partial "Timeout or API not reachable. Continuing to next step."
  assert_output --partial "Build created:"
  assert_success
...
@test "Continues when subscription API returns 200 for private repo" {
  ...
  stub curl \
    "--max-time 3 -s -w %{http_code} -X POST -H * -d * https://agent.api.stepsecurity.io/v1/github/buildkite/test-repo/actions/maintained-actions-subscription -o /dev/null : echo 200" \
    "--silent --show-error --write-out %{http_code} --output * -X POST -H \"Authorization: Bearer 123\" -d '$EXPECTED_JSON' https://api.buildkite.com/v2/organizations/my-org/pipelines/my-pipeline/builds : output_file=\"\"; for i in \"\$@\"; do if [ \"\$output_file\" = \"next\" ]; then echo '$RESPONSE_JSON' > \"\$i\"; break; fi; [ \"\$i\" = \"--output\" ] && output_file=\"next\"; done; echo 200"

  run "${PWD}"/entrypoint.sh

  refute_output --partial "This action requires a StepSecurity subscription"
  assert_output --partial "Build created:"
  assert_success
...
@test "Fails when GITHUB_OUTPUT is not set" {
  ...
  assert_output --partial "Error: GITHUB_OUTPUT is not set"
  assert_failure
