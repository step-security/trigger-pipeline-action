...
REPO_PRIVATE="${REPO_PRIVATE:-$(jq -r '.repository.private | tostring' "$GITHUB_EVENT_PATH" 2>/dev/null || echo "")}"
UPSTREAM="buildkite/trigger-pipeline-action"
...
if [[ -n "${GITHUB_OUTPUT:-}" ]]; then
  ...
  echo "Error: GITHUB_OUTPUT is not set. Please use a GitHub Actions runner that supports GITHUB_OUTPUT."
  exit 1
fi
